...
describe("product surface", () => {
  it("communicates through intake lanes, conveyors, a ratio gauge, paired bands, and three-year strips", () => {
    expect(worker).toContain('class="intake-board"');
    expect(worker).toContain('class="intake-lane is-opening"');
    expect(worker).toContain('class="intake-lane is-seeker"');
    expect(worker).toContain('class="conveyor"');
    expect(worker).toContain('class="intake-gauge"');
  ...
  it("states source, zero-denominator, classification, and interpretation boundaries", () => {
    ...
    expect(worker).toContain("新規申込が0のセルは無限大と表示しません");
    ...
    const accepted = await app.request(
      `${origin}/api/telemetry`,
      {
        ...
        headers: {
          ...
          origin,
          "x-shinki-bairitsu-session": "12345678-1234-4123-8123-123456789abc",
          "x-shinki-bairitsu-qa": "1",
        },
  ...
  it("separates QA, honors privacy signals, and needs no account", () => {
    ...
    expect(client).toContain('"x-shinki-bairitsu-qa"');
