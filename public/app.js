const MAX_COMPARE = 4;
const STORAGE_KEY = "shinki-bairitsu:compare:v1";
...
const getSession = () => {
  const key = "shinki-bairitsu:session:v1";
...
const track = (name) => {
  ...
  fetch("/api/telemetry", {
    ...
    headers: {
      ...
      "x-shinki-bairitsu-session": getSession(),
      "x-shinki-bairitsu-qa": isQa() ? "1" : "0",
    },
...
function intakeBand(pair, label) {
  ...
  return `<div aria-label="${escapeHtml(label)}" class="balance-meter" role="img"><span class="opening-side" style="width:${openingWidth.toFixed(2)}%"><i>新規求人</i></span><span class="seeker-side" style="width:${(100 - openingWidth).toFixed(2)}%"><i>新規申込</i></span><b aria-hidden="true"></b></div>`;
}
function yearStrip(placeId) {
  ...
  return `<div class="year-strip">${index.years
    .map((value, i) => {
      ...
      return `<div class="year-cell${value === Number(year.value) ? " is-current" : ""}"><span>${value}</span><b>${displayRatio(pair)}</b><small>新規求人 ${shortCount(pair[0])} · 新規申込 ${shortCount(pair[1])}</small></div>`;
    })
    .join("")}</div>`;
}
...
function renderCompare() {
  ...
  compareList.innerHTML = places
    .map((place) => {
      ...
      return `<article class="compare-card"><div class="compare-title"><div><span>${escapeHtml(place.region)}</span><strong>${escapeHtml(place.name)}</strong></div><button aria-label="${escapeHtml(place.name)}を比較から外す" data-remove="${place.id}" type="button">×</button></div><div class="compare-value"><span>${escapeHtml(selectedOccupation().name)}</span><b>${displayRatio(pair)}</b></div>${intakeBand(pair, `${place.name}の新規求人${displayCount(pair[0])}、新規申込${displayCount(pair[1])}`)}<dl class="count-pair"><div><dt>新規求人数</dt><dd>${displayCount(pair[0])}</dd></div><div><dt>新規求職申込件数</dt><dd>${displayCount(pair[1])}</dd></div></dl>${yearStrip(place.id)}<p class="coverage-note">${coverageLabel(place.id)}</p></article>`;
    })
    .join("");
}
      ...
      const ratioSentence =
        ratio === null
          ? pair[1] === 0 && pair[0] !== null
            ? "新規申込が0のため算出しません"
            : "同じ条件の2値がそろっていません"
          : `新規申込1件に対して新規求人 ${ratioNumber.format(ratio)}`;
      return `<article class="place-card"><div class="place-heading"><div><p>${escapeHtml(place.region)} · ${escapeHtml(place.id)}</p><h3>${escapeHtml(place.name)}</h3></div><strong>${displayRatio(pair)}</strong></div>${intakeBand(pair, `${place.name}の新規求人${displayCount(pair[0])}、新規申込${displayCount(pair[1])}`)}<p class="ratio-sentence">${ratioSentence}</p><dl class="place-counts"><div><dt>新規求人数</dt><dd>${displayCount(pair[0])}</dd></div><div><dt>新規求職申込件数</dt><dd>${displayCount(pair[1])}</dd></div></dl><p class="coverage-note">${coverageLabel(place.id)}</p><button class="compare-button${active ? " is-selected" : ""}" data-select="${place.id}" ${disabled ? "disabled" : ""} type="button">${active ? "比較中" : disabled ? "4地域を選択済み" : "比較に追加"}</button></article>`;
    })
    .join("");
}
function renderAll() {
  metricNote.textContent = `${employmentLabel()}で年度内に新たに受け付けた公式値です。同じ職種・地域・年度の新規求人 ÷ 新規申込で倍率を算出します。`;
  renderCompare();
  renderResults();
}
...
copyCompare.addEventListener("click", async () => {
  const lines = selected
    ...
    .map((place) => {
      const pair = currentPair(place.id);
      return `${place.name}｜${displayRatio(pair)}｜新規求人 ${displayCount(pair[0])}｜新規申込 ${displayCount(pair[1])}`;
    });
  await navigator.clipboard.writeText(
    [
      `${selectedOccupation().name}（${employmentLabel()}・${year.value}年度）`,
      ...lines,
      "年度内の新規受付件数。新規求人数は採用予定人員で、新規申込は固有人数と一致するとは限らず、分類不能の申込は職種別倍率に含まれません。",
